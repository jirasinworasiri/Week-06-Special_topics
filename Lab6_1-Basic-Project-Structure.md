...
1. **Docker vs Native Setup**: อธิบายข้อดีของการใช้ Docker เปรียบเทียบกับการติดตั้ง ESP-IDF บน host system 
  Docker ทำให้ทุกเครื่องมีสภาพแวดล้อมพัฒนาเหมือนกัน ลดปัญหาเวอร์ชันไม่ตรงและติดตั้งง่าย ขณะที่ native setup เร็วกว่าและเข้าถึงฮาร์ดแวร์ได้ตรงแต่ต้องดูแลการติดตั้งและแก้ปัญหาความเข้ากันเอง

2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
  น Docker เมื่อรัน idf.py build ระบบจะใช้ CMakeLists.txt กำหนดการคอมไพล์ CMake จะ generate ไฟล์ build ผ่าน Ninja คอมไพล์ source เป็น object files ลิงก์เป็น .elf และแปลงเป็น .bin สำหรับ flash

3. **CMake Files**: บทบาทของไฟล์ CMakeLists.txt แต่ละไฟล์คืออะไร และทำงานอย่างไรใน Docker environment?
  CMakeLists.txt ใน root กำหนดค่าหลักและเชื่อมกับ ESP-IDF build system ส่วนใน main/ และโมดูลต่าง ๆ ระบุไฟล์ source และ dependencies ซึ่ง Docker ใช้ร่วมกับ toolchain ใน container เพื่อจัดการ build

4. **Git Ignore**: ไฟล์ .gitignore มีความสำคัญอย่างไรสำหรับ ESP32 project development?
  .gitignore ป้องกันไม่ให้ไฟล์ build, ไฟล์ชั่วคราว และไฟล์เฉพาะเครื่องถูก commit ทำให้ repository สะอาดและลดปัญหาความต่างระหว่างเครื่องพัฒนา

5. **Container Persistence**: ข้อมูลใดบ้างที่จะหายไปเมื่อ restart container และข้อมูลใดที่จะอยู่ต่อ?
  ข้อมูลใน container layer จะหายเมื่อ restart ยกเว้นไฟล์ที่อยู่ใน volume หรือ bind-mount กับ host ซึ่งจะคงอยู่

6. **Development Workflow**: เปรียบเทียบ workflow การพัฒนาระหว่างการใช้ Docker กับการทำงานบน native system
  Docker เหมาะกับงานทีมและสภาพแวดล้อมคงที่แม้ build ช้ากว่าและต้องตั้งค่า USB pass-through ส่วน native ทำงานเร็วและเข้าถึงอุปกรณ์ได้ตรงแต่เสี่ยงปัญหาเวอร์ชันและต้องดูแลระบบเองมากกว่า
