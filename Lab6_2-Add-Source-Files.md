...
1. **Multiple Source Files**: เหตุใดต้องแยก source code เป็นหลายไฟล์?
    เพื่อแยกส่วนของโค้ดให้ง่ายต่อการจัดการและบำรุงรักษา
    ช่วยให้ทีมงานทำงานร่วมกันได้สะดวกขึ้น
    เพิ่มความชัดเจนและลดความซับซ้อนของโปรเจกต์

2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง?
    ต้องเพิ่มชื่อไฟล์ source ใหม่เข้าไปในตัวแปร SRCS หรือส่วนที่กำหนดไฟล์ source
    ระบุ path ของไฟล์ header หากอยู่ในโฟลเดอร์พิเศษผ่าน INCLUDE_DIRS

3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี?
    ประกาศฟังก์ชัน ตัวแปร และโครงสร้างข้อมูลให้ไฟล์อื่นรู้จัก
    ช่วยให้แบ่งการเขียนโค้ดเป็นส่วน ๆ และป้องกันการเขียนซ้ำ

4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS?
    เพื่อบอกคอมไพเลอร์ว่าจะไปหาไฟล์ header ที่ไหน
    ทำให้การเชื่อมโยงระหว่างไฟล์ source กับ header ถูกต้องและง่ายขึ้น

5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project?
    ช่วยบอก Git ว่าไฟล์หรือโฟลเดอร์ไหนไม่ต้องติดตาม (เช่น ไฟล์ build, ไฟล์ชั่วคราว)
    ลดขนาด repository และป้องกันการส่งไฟล์ที่ไม่จำเป็น

6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?
    ช่วยแยกงาน LED กะพริบให้อยู่ในงานเฉพาะ (task) แยกต่างหาก
    ทำให้ระบบสามารถทำงานหลายอย่างพร้อมกันได้อย่างมีประสิทธิภาพ

7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร?
    เพิ่มความชัดเจนของแต่ละฟังก์ชันงาน
    ง่ายต่อการแก้ไขและทดสอบแต่ละส่วน
    สนับสนุนการพัฒนาทีมและการนำกลับมาใช้ซ้ำ
...
- จำนวนไฟล์ source: 2 
- ขนาด binary:163,472 bytes
- การทำงาน: โปรแกรมอ่านค่าเซนเซอร์ (เช่น อุณหภูมิและความชื้น) จากโมดูล sensor.c แล้วส่งข้อมูลนี้ไปยัง main.c เพื่อแสดงผลในลูป โดยมีการแสดงผลซ้ำๆ หลายรอบ (loop) พร้อมการตรวจสอบสถานะเซนเซอร์ว่าทำงานปกติ

**ขั้นตอนที่ 2 (เพิ่ม display.c):**
- จำนวนไฟล์ source: 3
- ขนาด binary: 164,356 bytes
- การทำงาน: แสดงข้อมูลจากเซนเซอร์บนหน้าจอ แสดงข้อความสถานะและค่าต่าง ๆ โดยทำงานร่วมกับ main.c และ sensor.c อย่างเป็นระบบในแต่ละรอบของลูป

**ขั้นตอนที่ 3 (เพิ่ม led.c):**
- จำนวนไฟล์ source: 4
- ขนาด binary: 165,304 bytes
- การทำงาน: จัดการสถานะ LED เช่น เปิด-ปิดและกระพริบ พร้อมแสดงสถานะของ LED บนหน้าจอแสดงผล โดยทำงานประสานกับโมดูล sensor และ display รวมทั้ง main.c ในการอ่านข้อมูลเซนเซอร์ แสดงผลข้อมูล และแสดงสถานะ LED ในแต่ละรอบลูปอย่างต่อเนื่อง

**สังเกต:**
- การเพิ่มไฟล์ source ส่งผลต่อขนาด binary อย่างไร?
    การเพิ่มไฟล์ source ส่งผลให้ขนาด binary เพิ่มขึ้นตามขนาดและจำนวนโค้ดของไฟล์ที่เพิ่มเข้ามา
- LED กะพริบทุกกี่วินาที?
    กะพริบทุกประมาณ 1 วินาที
- แต่ละโมดูลแสดงข้อมูล file และ line อย่างไร?
    แต่ละโมดูลแสดงข้อมูลโดยบอกชื่อไฟล์และหมายเลขบรรทัดที่ทำงาน เพื่อช่วยติดตามและแก้ไขโค้ดได้ง่าย
